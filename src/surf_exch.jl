function surf_exch(ebm::EBM, cn::Constants, Ta, Ua)
    ...
    z0h = 0.1 * ebm.z0
    CD = (cn.vkman / log(ebm.zU / ebm.z0))^2.0
    ebm.CH = cn.vkman^2.0 / (log(ebm.zU / ebm.z0) * log(zTs / z0h))

    # Stability correction (Louis et al. 1982, quoted by Beljaars 1992)
    if (ebm.em == 1)
        RiB = cn.g * (Ta - ebm.Tsurf) * ebm.zU^2.0 / (zTs * Ta * Ua^2.0)
        if (RiB > 0.0)
