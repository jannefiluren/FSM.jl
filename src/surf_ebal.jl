function surf_ebal(ebm::EBM, Ta, Qa, Ua, Ps, SW, LW)
    ...
    ebm.LEsrf = Lh * Esurf
    ebm.Melt = 0.0
    ebm.Rnet = (1.0 - ebm.alb) * SW + LW - sb * ebm.Tsurf^4.0
    dTs = (ebm.Rnet - ebm.Hsurf - ebm.LEsrf - ebm.Gsurf) / ((cp + Lh * psi * D) * rKH + 2 * ebm.ksurf / ebm.Dz1 + 4.0 * sb * ebm.Tsurf^3.0)
    dE = psi * rKH * D * dTs
    dG = 2.0 * ebm.ksurf * dTs / ebm.Dz1
    dH = cp * rKH * dTs
    dR = -sb * ebm.Tsurf^3.0 * dTs

    # Surface melting
    if (ebm.Tsurf + dTs > Tm && ebm.Sice[1] > 0.0)
        ebm.Melt = sum(ebm.Sice) / ebm.dt
        dTs = (ebm.Rnet - ebm.Hsurf - ebm.LEsrf - ebm.Gsurf - Lf * ebm.Melt) / ((cp + Ls * psi * D) * rKH + 2.0 * ebm.ksurf / ebm.Dz1 + 4.0 * sb * ebm.Tsurf^3.0)
        dE = rKH * D * dTs
        dG = 2.0 * ebm.ksurf * dTs / ebm.Dz1
        dH = cp * rKH * dTs
        if (ebm.Tsurf + dTs < Tm)
            Qs = qsat(false, Ps, Tm)
            Esurf = rKH * (Qs - Qa)
            ebm.Gsurf = 2.0 * ebm.ksurf * (Tm - ebm.Ts1) / ebm.Dz1
            ebm.Hsurf = cp * rKH * (Tm - Ta)
            ebm.LEsrf = Ls * Esurf
            ebm.Rnet = (1.0 - ebm.alb) * SW + LW - sb * Tm^4.0
    ...
    ebm.Esnow = 0.0
    Esoil = 0.0
    if (ebm.Sice[1] > 0.0 || ebm.Tsurf < Tm)
        ebm.Esnow = Esurf
        ebm.LEsrf = Ls * Esurf
    else
        Esoil = Esurf
