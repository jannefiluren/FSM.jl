function surf_ebal(ebm::EBM, cn::Constants, Ta, Qa, Ua, Ps, SW, LW)
    ...
    dTs = (ebm.Rnet - ebm.Hsurf - ebm.LEsrf - ebm.Gsurf) / ((cn.cp + Lh * psi * D) * rKH + 2 * ebm.ksurf / ebm.Dz1 + 4 * cn.sb * ebm.Tsurf^3)
    dE = psi * rKH * D * dTs
    ...
    # Surface melting
    if (ebm.Tsurf + dTs > cn.Tm && ebm.Sice[1] > 0)
        ebm.Melt = sum(ebm.Sice) / ebm.dt
        dTs = (ebm.Rnet - ebm.Hsurf - ebm.LEsrf - ebm.Gsurf - cn.Lf * ebm.Melt) / ((cn.cp + cn.Ls * psi * D) * rKH + 2 * ebm.ksurf / ebm.Dz1 + 4 * cn.sb * ebm.Tsurf^3)
        dE = rKH * D * dTs
        dG = 2 * ebm.ksurf * dTs / ebm.Dz1
        dH = cn.cp * rKH * dTs
        if (ebm.Tsurf + dTs < cn.Tm)
            ...
            Esurf = rKH * (Qs - Qa)
            ebm.Gsurf = 2 * ebm.ksurf * (cn.Tm - ebm.Ts1) / ebm.Dz1
            ebm.Hsurf = cn.cp * rKH * (cn.Tm - Ta)
            ebm.LEsrf = cn.Ls * Esurf
            ebm.Rnet = (1 - ebm.alb) * SW + LW - cn.sb * cn.Tm^4
    ...
    ebm.Esnow = 0
    Esoil = 0
    if (ebm.Sice[1] > 0 || ebm.Tsurf < cn.Tm)
        ebm.Esnow = Esurf
        ebm.LEsrf = cn.Ls * Esurf
    else
        Esoil = Esurf
